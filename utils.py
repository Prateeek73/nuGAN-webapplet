...
def draw_latent_z(prior="gaussian", sample_shape=(100, 100), device="cpu", df=None):
    if prior == "studentT":
        z = (
            torch.distributions.StudentT(df=df, loc=0.0, scale=1.0)
...
def get_runtime(t):
    ...
    if divmod(t, 3600)[0] == 0:  # if no hours in time
        ...
        hours, mins_secs = divmod(
            t, 3600
        )  # else get hours and minutes_seconds together
...
def compute_gradient_penalty(D, real_samples, params, mchn, fake_samples, device):
    ...
    if real_samples.ndim == 5:
        alpha = torch.Tensor(np.random.random((real_samples.size(0), 1, 1, 1, 1))).to(
            device
        )
    ...
    fake = torch.autograd.Variable(
        torch.Tensor(real_samples.shape[0], 1).fill_(1.0), requires_grad=False
    ).to(device)
...
def spectral_loss_2d(real, fake, eps=1e-6, weighting="uniform"):
    """
    ...
    if np.isnan(pkbins).any():
        ...
        if valid.any():
            pkbins[~valid] = np.interp(
                np.arange(nbins)[~valid], np.arange(nbins)[valid], pkbins[valid]
            )
    return pkbins


def get_avg_pk_loss(real, fake, nbins=100, a_scale=4):
    """
    ...
    for i in range(real.shape[0]):
        ...
        try:
            Pk2D_real = PKL.Pk_plane(
                real_delta, BoxSize=500, MAS="CIC", threads=8, verbose=False
            )
            Pk2D_fake = PKL.Pk_plane(
                fake_delta, BoxSize=500, MAS="CIC", threads=8, verbose=False
            )

            # Bin the power spectra
            pkbins_r = get_binned_2D(Pk2D_real.k, Pk2D_real.Pk, nbins)
            pkbins_f = get_binned_2D(Pk2D_fake.k, Pk2D_fake.Pk, nbins)
